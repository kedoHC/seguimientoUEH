...
$peticion = "SELECT * FROM casoexito WHERE id_caso='".$_GET['id']."'";
...
<a href="admin.php">
					...
					<div id="contenedor-derecho" class="col-xs-10">
						<div class="row">
							<h1 class="text-center">Editar Caso de Éxito</h1>
							...
							<div id="nombre-div" class="row">
								...
								<div class="col-xs-5">
									...
									while($fila = mysqli_fetch_array($resultado)){  
										echo'
										<form name="formulariocasoexito" enctype="multipart/form-data" action="actualizarcasoexito.php?id='.$fila["id_caso"].'" method="POST">
										<input name="nombrecasoexito" maxlength="37" type="text" value="'.$fila["nombre_caso"].'">
										';
									?>
								</div>
							</div>
		...
		function confirmar(){
			...
			swal({
			  title: "¿Estas seguro?",
			  text: "Se eliminara para siempre el caso de éxito!",
			  type: "warning",
			...
			function(isConfirm) {
			  if (isConfirm) {
			    swal("¡Eliminado!", "El casod de éxito va a ser eliminado.", "success");
			    window.location.href = 'borrar-caso.php?id='+id+'';
			  } else {
			    swal("¡Cancelado!", "Caso de éxito a salvo :)", "error");
			  }
			});
		}
